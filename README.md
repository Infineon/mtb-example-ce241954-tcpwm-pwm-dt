# PSOC&trade; 4: TCPWM dead-time

This code example shows a timer function using timer, counter and pulse-width modulator (TCPWM) dead-time mode function. As per the PWM dead-time mode, the PWM line pin and PWM complementary line pin output signals are based on the 2 kHz frequency. The signal is output from the line pin with a delay of 25 µs from the rising edge of the 2 kHz based PWM and from the complementary line pin with a delay of 25 µs from the falling edge.

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce241954-tcpwm-pwm-dt)

[Provide feedback on this code example.](https://cypress.co1.qualtrics.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce241954-tcpwm-pwm-dt](https://github.com/Infineon/mtb-example-ce241954-tcpwm-pwm-dt)" application with the desired name "TcpwmDeadTime" configured for the *KIT_PSOC4-HVMS-128K_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSOC4-HVMS-128K_LITE --app-id mtb-example-ce241954-tcpwm-pwm-dt --user-app-name TcpwmDeadTime --target-dir "C:/mtb_projects"
   ```
...
3. After programming, the application starts automatically and outputs signals from the PWM line pin (P0.2: yellow line signal) and PWM complementary line pin (P0.3: blue line) as the PWM dead-time mode, which is shown in **Figure 1** and **Figure 2** respectively. For KIT_PSOC-HVPA-SPM1_LITE, it is the PWM line pin (P3.1) and the PWM complementary line pin (P0.5).

   **Figure 1. PWM dead-time mode in PWM line pin output (yellow line signal)**

   <img src="./images/pwm_dead_time_small1.png" width="500">

   <br>

   **Figure 2. PWM dead-time mode in PWM complementary line pin output (blue line signal)**

   <img src="./images/pwm_dead_time_small2.png" width="500">
...
1. Initializes to TCPWM PWM for dead-time mode 
2. Enables and starts to TCPWM PWM 
3. TCPWM PWM outputs signals to the PWM line and complementary line in dead-time mode
...
:------- | :------------    | :------------ 
TCPWM | TCPWM | To perform PWM dead-time mode
GPIO | P0_5 (KIT_PSOC-HVPA-SPM1_LITE) <br> P0_3 (Other boards)  | Shows PWM complementary line output signal
GPIO | P3_1 (KIT_PSOC-HVPA-SPM1_LITE) <br> P0_2 (Other boards) | Shows PWM line output signal
...
Document title: *CE241954* – *PSOC&trade; 4: TCPWM dead-time*
